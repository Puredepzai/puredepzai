...
export function applyContainerTransform(bytes, view) {
    ...
    const moovBoxes = parseBoxes(bytes, view, moovBox.offset + getBoxHeaderSize(moovBox), moovBox.end);
    ...
    for (const trakBox of trakBoxes) {
        const trakBoxes2 = parseBoxes(bytes, view, trakBox.offset + getBoxHeaderSize(trakBox), trakBox.end);
        const mdiaBox = trakBoxes2.find(b => b.type === 'mdia');
        const tkhdBox = trakBoxes2.find(b => b.type === 'tkhd');

        if (!mdiaBox || !tkhdBox) continue;

        const mdiaBoxes = parseBoxes(bytes, view, mdiaBox.offset + getBoxHeaderSize(mdiaBox), mdiaBox.end);
        ...
        if (mdhdVersion === 0) {
            ...
            mdhdDuration = Number(view.getBigUint64(mdhdContentStart + 24, false));
        }

        const minfBox = mdiaBoxes.find(b => b.type === 'minf');
        if (!minfBox) continue;

        const minfBoxes = parseBoxes(bytes, view, minfBox.offset + getBoxHeaderSize(minfBox), minfBox.end);
        const stblBox = minfBoxes.find(b => b.type === 'stbl');

        if (!stblBox) continue;

        const stblBoxes = parseBoxes(bytes, view, stblBox.offset + getBoxHeaderSize(stblBox), stblBox.end);
        ...
        for (let i = 0; i < sttsEntryCount; i++) {
            const sampleCount = view.getUint32(sttsContentStart + 8 + i * 8, false);
            const sampleDelta = view.getUint32(sttsContentStart + 12 + i * 8, false);
    ...
    for (const track of tracks) {
        ...
        track.newTkhdDuration = Math.round(track.newMdhdDuration * timescaleRatio * (mvhdTimescale / track.mdhdTimescale));
    ...
    for (const track of tracks) {
        // Copy trak
        newBytes.set(bytes.subarray(track.trakBox.offset, track.trakBox.offset + getBoxHeaderSize(track.trakBox)), writeOffset);
        const trakStartOffset = writeOffset;
        writeOffset += getBoxHeaderSize(track.trakBox);

        // Copy and patch tkhd
        newBytes.set(bytes.subarray(track.tkhdBox.offset, track.tkhdBox.end), writeOffset);
        const newTkhdContentStart = writeOffset + getBoxHeaderSize(track.tkhdBox);
        const tkhdVersion = bytes[track.tkhdBox.offset + getBoxHeaderSize(track.tkhdBox)];
        ...
        if (tkhdVersion === 0) {
            ...
            newView.setUint32(newTkhdContentStart + 20, track.newTkhdDuration, false);
        }

        writeOffset += track.tkhdBox.size;

        // Copy mdia
        newBytes.set(bytes.subarray(track.mdiaBox.offset, track.mdiaBox.offset + getBoxHeaderSize(track.mdiaBox)), writeOffset);
        writeOffset += getBoxHeaderSize(track.mdiaBox);

        // Copy and patch mdhd
        newBytes.set(bytes.subarray(track.mdhdBox.offset, track.mdhdBox.end), writeOffset);
        const newMdhdContentStart = writeOffset + getBoxHeaderSize(track.mdhdBox);
        const mdhdVersion = bytes[track.mdhdBox.offset + getBoxHeaderSize(track.mdhdBox)];
        ...
        if (mdhdVersion === 0) {
            ...
            newView.setUint32(newMdhdContentStart + 16, track.newMdhdDuration, false);
        ...
        // Copy minf
        newBytes.set(bytes.subarray(track.minfBox.offset, track.minfBox.offset + getBoxHeaderSize(track.minfBox)), writeOffset);
        writeOffset += getBoxHeaderSize(track.minfBox);

        // Copy stbl
        newBytes.set(bytes.subarray(track.stblBox.offset, track.stblBox.offset + getBoxHeaderSize(track.stblBox)), writeOffset);
        ...
        newView.setUint32(newSttsContentStart + 4, track.newSttsEntryCount, false);
        writeOffset += 8;
        ...
        const avgDelta = Math.round((track.actualDuration * 2) / track.sttsEntryCount);
        ...
        const stblSize = stblEndOffset - (stblEndOffset - track.stblBox.size - 8);
        newView.setUint32(stblEndOffset - track.stblBox.size - 8 - track.minfBox.size - track.mdhdBox.size - track.tkhdBox.size - getBoxHeaderSize(track.trakBox) - getBoxHeaderSize(track.mdiaBox) - getBoxHeaderSize(track.minfBox) - getBoxHeaderSize(track.stblBox), stblSize, false);
    }
