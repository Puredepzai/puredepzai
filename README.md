...
### Combined VFI + HDR Path (Two-Stage)

When both toggles are enabled, both pipelines run sequentially as two separate FFmpeg stages:
1. **Stage 1 (VFI):** Motion interpolation to 60fps, encoded as HEVC 8-bit (ultrafast, CRF 20).
2. **Stage 2 (HDR):** The VFI output is re-read and tone-expanded with PQ/BT.2020 filters, encoded as HEVC 10-bit (fast, CRF 18).

FFmpeg instances are destroyed and recreated between stages to prevent WASM memory exhaustion (OOM). Audio handling follows the same rules (copy or AAC 256k for MOV inputs).
...
- **60fps VFI Interpolation:** Motion-compensated frame interpolation to 60fps via FFmpeg.wasm, output as HEVC 8-bit.
...
- **Two-Stage VFI+HDR:** When both toggles are enabled, sequential two-stage processing prevents WASM memory crashes while delivering 60fps HDR10 output.
